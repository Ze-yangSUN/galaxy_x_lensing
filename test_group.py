...
def load_SELECTION_less(zmin,zmax,mass):
    t0 = time.time()
    print ('======================== %s<z<%s, mass<%s ========================'%(zmin,zmax,mass))
    filename = '../desi_data/SELECTION/groups_%sto%s_less_M%s.fits'%(zmin,zmax,mass)
...
def load_SELECTION_moreeq(zmin,zmax,mass):
    t0 = time.time()
    print ('======================== %s<z<%s, mass>=%s ========================'%(zmin,zmax,mass))
    filename = '../desi_data/SELECTION/groups_%sto%s_moreeq_M%s.fits'%(zmin,zmax,mass)
    f = fio.FITS(filename)[-1].read()
    ra = f['ra']
    dec = f['dec']
    M = f['M']
...
def fi_mask(threshold):
    from NPTFit import create_mask as cm
    nothrh = np.load('../desi_data/survey_mask/ud512_mask_nothrh.npy')
    mask_thrh = np.load('../desi_data/survey_mask/ud512_mask_thrh%s.npy'%threshold)
    fi = nothrh * mask_thrh
    return fi, mask_thrh
...
#np.save('../desi_data/act_survey_mask/mask_thrh%s_%s.npy'%(threshold,patch), mask)
#hp.mollview(mask)
#plt.savefig('../desi_data/act_survey_mask/mask_thrh%s_%s.png'%(threshold,patch))
#K_ra, K_dec, K_val, K_mask, K_lm, N_lm, K, Nmap = load_CMB(tSZ=tSZ)
#np.save('../desi_data/survey_mask/K_mask_tszTrue.npy',K_mask)
...
def after_masking(id,Nmember,ra,dec,photo_z,M,L,threshold):
    t0=time.time()
    g_fi, g_mask, mask = total_mask(threshold)
    ...
    arr1 = np.array([pxl,id,Nmember,ra,dec,photo_z,M,L])
    print ('****************** START MASKING *****************')
...
def after_MASK(id,Nmember,ra,dec,photo_z,M,L,threshold):
    t0=time.time()
    g_fi, g_mask, mask = total_mask(threshold)
    mask_pxl = np.array(np.where(mask==0))[0] #masked pixel index    
    ra,dec = fk5_2_galactic(ra,dec)
...
def load_AFTER_MASK(loc,lab,z):
    print ('==================%s, %s =================='%(lab,z))
    t0 = time.time()
    filename = '../desi_data/SELECTION/%s/arr_%s_z%s.fits'%(loc,lab,z)
    f = fio.FITS(filename)[-1].read()
    ra = f['ra']
    dec = f['dec']
    M = f['M']
...
def load_AFTER_MASK_z(loc, z):
    print ('==================z%s==================='%z) 
    t0 = time.time()
    filename='../desi_data/SELECTION/%s/arr_z%s.fits'%(loc, z)
    f = fio.FITS(filename)[-1].read()
    print ('===> time to load AFTER MASK =', time.time()-t0)
    return f


def load_AFTER_MASK_array(loc):
    print ('==================array ==================')
    t0 = time.time()
    filename = '../desi_data/SELECTION/%s/arr.fits'%(loc)
    f = fio.FITS(filename)[-1].read()
    ra = f['ra']
    dec = f['dec']
    M = f['M']
...
def counts_group(ra,dec,nside=512):
    t0=time.time()
...
def fill_mask():
    ...
    ra,dec = fk5_2_galactic(ra, dec)
    print ('ra,ra.shape=',ra,ra.shape)
    colorpix, meanfield = counts_group(ra,dec,nside=2048)
    np.save('../desi_data/survey_mask/colorpix_%s.npy'%nside,colorpix)
    slt = colorpix > 0
    mask = np.zeros(hp.nside2npix(nside=2048))
    mask[slt] = 1 
    return mask


# old delta
#def modified_map(colorpix,threshold): 
#    t0=time.time()
#    fi,g_mask = fi_mask(threshold) # 0.5~1
#    mask = total_mask(threshold)
#    ans = np.where(fi!=0)
#    density = colorpix*g_mask
#    mod_mean = np.sum(density) / np.sum(fi)
#    density[ans] = density[ans] / fi[ans]
#    print ('---> the mean value of modified overdensity map=',mod_mean)
...
def clkg_test(Modified,threshold):
    t0=time.time()
    K_ra, K_dec, K_val, K_mask, K_lm, N_lm, K, Nmap = load_CMB(tSZ=tSZ)
    g_fi, g_mask, mask = total_mask(threshold)
    kg = hp.anafast(K*mask, Modified*mask)
